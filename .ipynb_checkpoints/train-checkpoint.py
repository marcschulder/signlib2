...
def lstm_train(window, data_h5):
    ...
    for l in lstm_dim:
        ...
        history = model.fit(X_train, y_train, batch_size=32, epochs=128, verbose = 0, shuffle=False, validation_split=0.2, callbacks=[es])
...
#         print(classification_report(y_test, y_pred, target_names=target_names))
        mf = mf.append(pd.DataFrame({"Window": window, "lstm_dim": l, "Test_score": score, "Test_accuracy": acc*100 }, index=[0]))
...
#                                                  y_train[:,0])
    ...
    for l in lstm_dim:
        ...
        mf = mf.append(pd.DataFrame({"Window": window, "lstm_dim": l, "Test_score": acc, "Test_accuracy": roc }, index=[0]))
